...
void correlation::getTel()
{
  ...
  m_x = m_TAbc->m_x;
  m_y = m_TAbc->m_y;
  ntracks = m_TAbc->ntracks;
  distance = m_TAbc->distance;
  m_strip_pos = m_TAbc->m_strip_pos;
  m_isololation = m_TAbc->getIisolationParameter();
}

void correlation::getABC()
{
  m_cl_address = m_TAbc->m_cl_address;

  m_threshold = m_TAbc->m_threshold;
  abc_hits = m_TAbc->abc_hits;
  cl_size = m_TAbc->cl_size;
}
...
void correlation::open(const char* name,TFile* f)
{
  ...
  m_tree->Branch("isolating", &m_isololation, "m_isolating/D");
  eventNR = 0;
}
